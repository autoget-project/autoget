...
1. **Stage 1 Classifier** (`internal/pipeline/stage1_classifier/`) — determines the media `Category`. This is the ONLY stage allowed to do lightweight pattern matching (pure ebook/audio extensions, standard bango IDs like `^[A-Z]{3,5}-\d{3,4}$`, valid `dmm_id`). Dirty/ambiguous/mixed inputs must fall through to the LLM classifier. During a replan, the coarse upstream `organizer_category` rule is deliberately ignored (it may encode the very mistake being corrected) while authoritative rules still apply.
